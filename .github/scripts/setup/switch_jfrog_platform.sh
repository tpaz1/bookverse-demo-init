...
validate_host_format() {
    ...
    if [[ ! "$NEW_JFROG_URL" =~ ^https://[a-zA-Z0-9.-]+\.jfrog\.io$ ]]; then
        log_error "Invalid host format. Expected: https://tompazus.jfrog.io"
...
update_all_repositories() {
    ...
    for repo in "${BOOKVERSE_REPOS[@]}"; do
        ...
        if gh repo clone "$GITHUB_ORG/$repo_name" repo >/dev/null 2>&1; then
            ...
            # Step 1: Replace full HTTPS URLs (https://tompazus.jfrog.io -> NEW_JFROG_URL)
            ...
            # Step 2: Replace registry-only references with word boundaries (tompazus.jfrog.io -> new_registry)
